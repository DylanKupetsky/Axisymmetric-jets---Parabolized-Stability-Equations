...
def LST(N): #Function that will generate matrices A and B that correspond to a spatial LST using the mean functions
    ...
    twoOneA = Wrmat
    twoTwoA = -1j*np.identity(N + 1)*omega

    twoTwoB = -1j*Wmat
 
    twoThreeB = -1j*np.identity(N + 1)

    threeOneA = -1j*np.identity(N + 1)*omega
    ...
    # newA[2*(N + 1) - 1,2*(N + 1) - 1] = 1 #w(r=-inf) = 0
    ...
    # newA[2*(N + 1) - 1] = np.zeros(3*(N + 1))
    # newA[2*(N + 1) - 1,1*(N + 1):2*(N + 1)] = derivmatrix[-1] 
    # newB[2*(N + 1) - 1] = np.zeros(3*(N + 1)) #dw/dr(r = 0) = 0
...
# %%
def T(alpha,j): #j is the index of z
    global omega
    global W
...
def L(alpha,j):
    ...
    # Lmat[2*(N + 1) - 1,2*(N + 1) - 1] = 1 #w(r=-inf) = 0
    Lmat[N + 1] = np.zeros(3*(N + 1))
    Lmat[N + 1][N + 1] = 1 #w(r=inf) = 0
    #on p#
    # twoTwo[-1] = derivmatrix[-]
    ### BOUNDARY CONDITIONS!! ###
    return Lmat
...
def M(j):
    ...
    threeTwo = oneOne.copy()
    # threeThree = np.identity(N + 1,dtype=complex)
    threeThree = zeromat.copy()
    ...
    #on w#
    # Mmat[2*(N + 1) - 1] = np.zeros(3*(N + 1)) #w(r=-inf) = 0
...
A = deltaZ*L(beta,j=1) + M(j=1)
b = np.matmul(M(j=1),qprev)
...
alphanew = beta - intg
print(alphanew)
relError = (np.sqrt(np.real(alphanew - beta)**2 + np.imag(alphanew - beta)**2))/np.sqrt((np.real(alphanew)**2 + np.imag(alphanew)**2))
...
for j in range(len(z) - 1):
    ...
    while relError > 10**(-5):
        ...
        A = deltaZ*L(alphaold,j=j + 1) + M(j = j + 1)
        b = np.matmul(M(j = j + 1),q[:,j])
        qnext = scipy.linalg.solve(A,b)
        ...
        relError = np.sqrt((np.real(alphanew - alphaold)**2 + np.imag(alphanew - alphaold)**2))/np.sqrt((np.real(alphanew)**2 + np.imag(alphanew)**2))
        print('Current alpha = {}, m = {}, Relative error = {}'.format(alphanew,m,relError))
        m += 1
    print('Convergence complete')
    q[:,j + 1] = qnext
    alphavec.append(alphanew)
    condvec.append(np.linalg.cond(L(alphanew,j=j + 1)*deltaZ + M(j=j+1)))
# %%
plt.style.use('dark_background')
def η(index):
    global z
...
#will be antisymmetric since positive u values on the
...
xieven = ((3)/(np.pi))*np.arccos((2/3)*(xi - .001) - 1)
...
ani.save('Streamline_jet_axisymmetric_t0to20pi.gif',writer=writergif)
# %%
...
# print(np.shape(xixi))
ax.contour(xixi,zz,Wab + vellevels,levels=50)
